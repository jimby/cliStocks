...
def report(conn, maid):
    cursor = conn.cursor()
    print(maid)
    sql = """select tp.effective_date, s.trans_date, a.long_acct, s.name, tp.symbol, \
        (100 * ((tp.prices - s.price) / s.price) / (DATEDIFF(tp.effective_date, s.trans_date) / 365.25)) as APR from\
        ((stocks s inner join accounts a on s.aid=a.aid) inner join test_prices tp on s.stock_symbol=tp.symbol) where s.aid = '%s'\
    ...
    for row in rows:
        print("{:%m/%d/%Y}".format(row[1]), "{:%m/%d/%Y}".format(row[0]), row[2], "{:35s}".format(row[3]),
...
def main():
    ...
    while True:
        ...
        while True:
            ...
            if yn == 'y' or yn == 'Y':
                break
        yn = input("Try another account (y/n) ?")
        if yn == 'n' or yn == 'N':
            break
